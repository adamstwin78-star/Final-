...
tabSettings:CreateParagraph({
    Title   = "How it works",
    Content = "Hooks __namecall and __index on UserInputService and GuiService to lie to the game about your platform at the API level. Console: spoof Xbox One gamepad. Mobile: spoof touch input. PC: restore defaults (removes spoof).",
})

------------------------------------------------------------------------
-- Platform Spoofer — hookmetamethod approach
------------------------------------------------------------------------
local plat = {
    enabled      = cfg.get("platEnabled", false),
    device       = cfg.get("platDevice",  "Console"),
    ncHook       = nil,
    idxHook      = nil,
    origNC       = nil,
    origIdx      = nil,
...
-- Profile tables per device
local platProfiles = {
    Console = {
        GetPlatform     = function() return Enum.Platform.XBoxOne end,
        IsTenFootInterface = function() return true end,
        GetPlatformName = function() return "XboxOne" end,
        TouchEnabled    = false,
        MouseEnabled    = false,
    ...
    Mobile = {
        GetPlatform     = function() return Enum.Platform.IOS end,
        IsTenFootInterface = function() return false end,
        GetPlatformName = function() return "IOS" end,
        TouchEnabled    = true,
        MouseEnabled    = false,
        KeyboardEnabled = false,
    ...
    PC = {
        ...
        IsTenFootInterface = function() return false end,
        GetPlatformName = function() return "Windows" end,
        TouchEnabled    = false,
        MouseEnabled    = true,
...
local function platApply()
    if not hookmetamethod then
        warn("[Frostsaken] Platform Spoofer: hookmetamethod not available in this executor.")
        return
    end
    if plat.ncHook then return end -- already hooked

    local profile = platProfiles[plat.device] or platProfiles["Console"]

    plat.origNC = hookmetamethod(game, "__namecall", function(self, ...)
        local method = getnamecallmethod()
        if self == UIS or self == GS then
            if method == "GetPlatform"        then return profile.GetPlatform() end
            if method == "IsTenFootInterface" then return profile.IsTenFootInterface() end
            if method == "GetPlatformName"    then return profile.GetPlatformName() end
        end
        return plat.origNC(self, ...)
    end)

    plat.origIdx = hookmetamethod(game, "__index", function(tbl, idx)
        if tbl == UIS then
            local cs = tostring(getcallingscript())
            if cs ~= "ControlModule" then
                if idx == "TouchEnabled"      then return profile.TouchEnabled      end
                if idx == "MouseEnabled"      then return profile.MouseEnabled      end
                if idx == "KeyboardEnabled"   then return profile.KeyboardEnabled   end
                if idx == "GamepadEnabled"    then return profile.GamepadEnabled    end
                if idx == "ControllerEnabled" then return profile.ControllerEnabled end
            end
        end
        return plat.origIdx(tbl, idx)
    end)
...
local function platRemove()
    -- hookmetamethod hooks persist until re-hooked; we restore by re-hooking with passthrough
    if not plat.origNC or not plat.origIdx then return end
    hookmetamethod(game, "__namecall", function(self, ...)
        return plat.origNC(self, ...)
    end)
    hookmetamethod(game, "__index", function(tbl, idx)
        return plat.origIdx(tbl, idx)
    end)
    plat.ncHook  = nil
    plat.idxHook = nil
...
local function platRefresh()
    -- when device changes while active: remove and re-apply with new profile
    if not plat.enabled then return end
    platRemove()
    platApply()
end

if plat.enabled then
    task.delay(1, platApply) -- delay so game services are ready
end

tabSettings:CreateToggle({
    Name = "Enable Spoofer", CurrentValue = plat.enabled, Flag = "platEnabled",
    Callback = function(on)
        plat.enabled = on
        cfg.set("platEnabled", on)
        if on then platApply() else platRemove() end
    end,
})
tabSettings:CreateDropdown({
    Name = "Device", Options = { "Console", "Mobile", "PC" }, CurrentOption = plat.device,
    Flag = "platDevice",
    Callback = function(v)
        plat.device = v
...
-- LAG FIX: cache character/backpack refs per-call instead of rebuilding every iteration
local function espItemHeld(obj)
    for _, plr in ipairs(svc.Players:GetPlayers()) do
        local ch = plr.Character
        if ch then
            if obj == ch or obj:IsDescendantOf(ch) then return true end
        end
...
local function espDoItems(on)
    ...
    -- LAG FIX: spread descendant scan across frames
    task.spawn(function()
        local desc = ig:GetDescendants()
...
-- LAG FIX: check exact equality before calling expensive IsDescendantOf
local function isRealPlayer(obj)
    for _, plr in ipairs(svc.Players:GetPlayers()) do
        local ch = plr.Character
        if ch then
            if obj == ch then return true end
            if obj:IsDescendantOf(ch) then return true end
        end
    end
    return false
end
...
local function addPuddleHighlight(part, color, tag, label)
    ...
    task.defer(function()
        ...
        -- LAG FIX: throttle position sync to ~10fps instead of every Heartbeat
        local _lastFollowTick = 0
        local followConn = svc.Run.Heartbeat:Connect(function()
            if not part.Parent then return end
            local now = tick()
            if now - _lastFollowTick < 0.1 then return end
            _lastFollowTick = now
            holder.CFrame = CFrame.new(part.Position + Vector3.new(0, 0.05, 0))
        end)
...
-- LAG FIX: GetDescendants is expensive — spread across frames with task.defer batching
local function scanPizza()
    if not mset.pizza then return end
    task.spawn(function()
        local desc = svc.WS:GetDescendants()
        for i, obj in ipairs(desc) do
            ...
            if i % 200 == 0 then task.wait() end -- yield every 200 to stay smooth
        end
    end)
end
